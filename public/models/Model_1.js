 ...
 */
function createVoxelTexture(w, h, primaryHex, secondaryHex, grain = 0.08, accentHex, glowPattern = false) {
 ...
 */
function createVoxelClaymoreSword(theme) {
...
export function createMinecraftCharacterModel(input) {
    let resolvedThemeId = 'cyber-paladin';
    let showSword = true;
    if (typeof input === 'string' && SKIN_THEMES[input]) {
        resolvedThemeId = input;
    }
    else if (typeof input === 'object' && input !== null) {
        if (input.theme && SKIN_THEMES[input.theme])
            resolvedThemeId = input.theme;
        else if (input.skinTheme && SKIN_THEMES[input.skinTheme])
            resolvedThemeId = input.skinTheme;
        else if (input.colorScheme && SKIN_THEMES[input.colorScheme])
            resolvedThemeId = input.colorScheme;
        if (input.showSword !== undefined)
            showSword = input.showSword;
    }
    ...
    [
        ...
        grp.add(pupil);
        // Catchlight Sparkle
        const sparkle = new THREE.Mesh(new THREE.PlaneGeometry(p * 0.5, p * 0.5), new THREE.MeshBasicMaterial({ color: 0xffffff }));
    ...
    const danceTracks = [
        ...
        new THREE.VectorKeyframeTrack('Node_PelvisPivot.position', [
        ...
        new THREE.NumberKeyframeTrack('Node_PelvisPivot.rotation[y]', [0.0, 1.25, 2.5, 3.75, 5.0, 5.8, 6.25, 7.5, 8.5, 9.5, 10.0], [0.0, 0.38, -0.38, 0.48, -0.48, Math.PI * 2, 0.0, 0.32, -0.32, 0.0, 0.0]),
        new THREE.NumberKeyframeTrack('Node_PelvisPivot.rotation[z]', [0.0, 0.625, 1.25, 1.875, 2.5, 3.75, 5.0, 6.25, 7.5, 8.5, 9.5, 10.0], [0.0, 0.14, -0.14, 0.14, 0.0, -0.18, 0.18, 0.0, 0.16, -0.16, 0.0, 0.0]),
        new THREE.NumberKeyframeTrack('Node_TorsoPivot.rotation[x]', [0.0, 1.25, 2.5, 5.0, 5.8, 7.5, 8.5, 9.5, 10.0], [0.0, 0.1, -0.05, 0.14, -0.22, 0.08, 0.25, 0.36, 0.0]),
        // Cape Aerodynamics
        new THREE.NumberKeyframeTrack('Node_CapeGroup.rotation[x]', [0.0, 0.625, 1.25, 2.5, 5.4, 5.8, 6.25, 7.5, 10.0], [0.15, 0.45, 0.12, 0.15, 0.95, 1.1, 0.25, 0.15, 0.15]),
        // Head Pitch & Nodding Rhythm
        new THREE.NumberKeyframeTrack('Node_HeadPivot.rotation[y]', [0.0, 1.25, 2.5, 3.75, 5.0, 6.25, 7.5, 8.75, 10.0], [0.0, -0.32, 0.32, -0.38, 0.38, 0.0, -0.25, 0.25, 0.0]),
        new THREE.NumberKeyframeTrack('Node_HeadPivot.rotation[x]', [0.0, 0.625, 1.25, 1.875, 2.5, 3.75, 5.0, 7.5, 8.5, 9.5, 10.0], [0.0, 0.2, -0.1, 0.2, 0.0, 0.25, -0.14, 0.14, -0.18, 0.28, 0.0]),
        new THREE.NumberKeyframeTrack('Node_HeadPivot.rotation[z]', [0.0, 1.25, 2.5, 3.75, 5.0, 7.5, 10.0], [0.0, -0.15, 0.15, 0.18, -0.18, 0.1, 0.0]),
        // Left Arm Free Dancing
        new THREE.NumberKeyframeTrack('Node_LeftArmPivot.rotation[x]', [0.0, 0.625, 1.25, 1.875, 2.5, 3.125, 3.75, 4.375, 5.0, 5.8, 6.875, 7.5, 8.5, 9.5, 10.0], [
            0.0, -1.75, 0.22, -1.75, 0.0,
            -2.5, -0.32, -2.5, -1.2,
            -3.14, 0.4, -1.5,
            -2.1, 0.4, 0.0
        ]),
        new THREE.NumberKeyframeTrack('Node_LeftArmPivot.rotation[z]', [0.0, 0.625, 1.25, 1.875, 2.5, 3.75, 5.0, 5.8, 7.5, 8.5, 9.5, 10.0], [0.08, 0.6, 0.16, 0.6, 0.08, 1.35, 0.32, 0.78, 1.15, 0.2, 0.08, 0.08]),
        // Right Arm Sword Slashes & Flourishes
        new THREE.NumberKeyframeTrack('Node_RightArmPivot.rotation[x]', [0.0, 0.625, 1.25, 1.875, 2.5, 3.125, 3.75, 4.375, 5.0, 5.8, 6.875, 7.5, 8.5, 9.5, 10.0], [
            0.0, 0.22, -1.75, 0.22, 0.0,
            -0.32, -2.5, -0.32, -2.5,
            -3.14, -1.5, 0.4,
            0.4, -2.1, 0.0
        ]),
        new THREE.NumberKeyframeTrack('Node_RightArmPivot.rotation[z]', [0.0, 0.625, 1.25, 1.875, 2.5, 3.75, 5.0, 5.8, 7.5, 8.5, 9.5, 10.0], [-0.08, -0.16, -0.6, -0.16, -0.08, -0.32, -1.35, -0.78, -0.2, -1.15, -0.08, -0.08]),
        // Left Leg Ground Planting & Kick Flares
        new THREE.NumberKeyframeTrack('Node_LeftLegPivot.rotation[x]', [0.0, 0.625, 1.25, 1.875, 2.5, 3.75, 5.0, 5.8, 6.25, 7.5, 8.5, 9.5, 10.0], [0.0, -0.7, 0.48, -0.7, 0.0, 0.45, -0.45, -1.2, 0.35, -0.6, 0.38, 0.09, 0.0]),
        new THREE.NumberKeyframeTrack('Node_LeftLegPivot.rotation[z]', [0.0, 1.25, 2.5, 3.75, 5.0, 7.5, 8.5, 10.0], [0.0, 0.25, -0.05, 0.3, 0.0, 0.2, -0.2, 0.0]),
        // Right Leg Motion
        new THREE.NumberKeyframeTrack('Node_RightLegPivot.rotation[x]', [0.0, 0.625, 1.25, 1.875, 2.5, 3.75, 5.0, 5.8, 6.25, 7.5, 8.5, 9.5, 10.0], [0.0, 0.48, -0.7, 0.48, 0.0, -0.45, 0.45, -0.85, -0.35, 0.6, -0.38, 0.09, 0.0]),
        new THREE.NumberKeyframeTrack('Node_RightLegPivot.rotation[z]', [0.0, 1.25, 2.5, 3.75, 5.0, 7.5, 8.5, 10.0], [0.0, 0.05, -0.25, 0.0, -0.3, -0.2, 0.2, 0.0]),
    ];
    ...
    const runtime = {
        ...
        setJointAngles(angles) {
            if (angles.headPitch !== undefined)
                headPivot.rotation.x = angles.headPitch;
            if (angles.headYaw !== undefined)
                headPivot.rotation.y = angles.headYaw;
            if (angles.headRoll !== undefined)
                headPivot.rotation.z = angles.headRoll;
            if (angles.torsoYaw !== undefined)
                torsoPivot.rotation.y = angles.torsoYaw;
            if (angles.torsoPitch !== undefined)
                torsoPivot.rotation.x = angles.torsoPitch;
            if (angles.torsoRoll !== undefined)
                torsoPivot.rotation.z = angles.torsoRoll;
            if (angles.leftArmPitch !== undefined)
                leftArmPivot.rotation.x = angles.leftArmPitch;
            if (angles.leftArmYaw !== undefined)
                leftArmPivot.rotation.y = angles.leftArmYaw;
            if (angles.leftArmRoll !== undefined)
                leftArmPivot.rotation.z = angles.leftArmRoll;
            if (angles.rightArmPitch !== undefined)
                rightArmPivot.rotation.x = angles.rightArmPitch;
            if (angles.rightArmYaw !== undefined)
                rightArmPivot.rotation.y = angles.rightArmYaw;
            if (angles.rightArmRoll !== undefined)
                rightArmPivot.rotation.z = angles.rightArmRoll;
            if (angles.leftLegPitch !== undefined)
                leftLegPivot.rotation.x = angles.leftLegPitch;
            if (angles.leftLegYaw !== undefined)
                leftLegPivot.rotation.y = angles.leftLegYaw;
            if (angles.leftLegRoll !== undefined)
                leftLegPivot.rotation.z = angles.leftLegRoll;
            if (angles.rightLegPitch !== undefined)
                rightLegPivot.rotation.x = angles.rightLegPitch;
            if (angles.rightLegYaw !== undefined)
                rightLegPivot.rotation.y = angles.rightLegYaw;
            if (angles.rightLegRoll !== undefined)
                rightLegPivot.rotation.z = angles.rightLegRoll;
        },
        setSkinTheme(newThemeId) {
...
export const createMinecraftCharacter = (options) => {
