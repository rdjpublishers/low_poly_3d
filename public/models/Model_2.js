 ...
 * ========================================================================= */
function createKitbashBox(w, h, d, mat, topScaleX = 1, topScaleZ = 1, shiftX = 0, shiftZ = 0, name = 'part') {
...
export function buildLowPolyWheel(radius, width, materials, isRight, prefix) {
...
export function buildSupercarBody(materials) {
    const bodyGroup = new THREE.Group();
    bodyGroup.name = 'supercar_body_assembly';
    function addPart(w, h, d, mat, topScaleX = 1, topScaleZ = 1, shiftX = 0, shiftZ = 0, x = 0, y = 0, z = 0, rx = 0, ry = 0, rz = 0, name = 'supercar_body_part') {
 ...
 * ========================================================================= */
function buildDriveClip() {
    const fps = 30;
    const duration = 2.0;
    const frameCount = Math.round(duration * fps);
    const times = [];
    const wheelSpinValues = [];
    const chassisPosValues = [];
    const chassisRotValues = [];
...
function buildIdleRevClip() {
    const duration = 1.2;
    const times = [0, 0.3, 0.6, 0.9, 1.2];
    const q = new THREE.Quaternion();
    const e = new THREE.Euler();
    const chassisRotVals = [];
    ...
    const tracks = [
        new THREE.VectorKeyframeTrack('Node_Chassis.position', times, [0, 0.38, 0, 0, 0.375, 0, 0, 0.384, 0, 0, 0.377, 0, 0, 0.38, 0]),
 ...
 * ========================================================================= */
export function createSupercarModel(options = {}) {
    ...
    const clips = [driveClip, idleRevClip, driftClip, parkedClip];
    ...
    carRoot.tick = (dt) => mixer.update(Math.min(dt, 0.1));
