...
function createProceduralNoiseTexture(width = 256, height = 256, type = 'wear') {
...
function createBeveledCylinder(radiusTop, radiusBottom, height, radialSegments = 24) {
    return new THREE.CylinderGeometry(radiusTop, radiusBottom, height, radialSegments);
}
function createKneeBladeSpurGeometry() {
...
function createTalonFootGeometry() {
    const shape = new THREE.Shape();
...
export function createSpiderSentryMechModel(options = {}) {
    ...
    const chassisRingOuter = new THREE.Mesh(new THREE.CylinderGeometry(0.72, 0.78, 0.32, 28), primaryPaint);
    baseChassis.add(chassisRingOuter);
    const chassisTopPlate = new THREE.Mesh(new THREE.CylinderGeometry(0.58, 0.68, 0.1, 28), gunmetal);
    chassisTopPlate.position.y = 0.18;
    baseChassis.add(chassisTopPlate);
    const chassisBottomPlate = new THREE.Mesh(new THREE.CylinderGeometry(0.68, 0.54, 0.12, 28), darkSteel);
    chassisBottomPlate.position.y = -0.18;
    baseChassis.add(chassisBottomPlate);
    const turntableBearing = new THREE.Mesh(new THREE.TorusGeometry(0.48, 0.045, 12, 32), polishedSteel);
    turntableBearing.rotation.x = Math.PI / 2;
    turntableBearing.position.y = 0.24;
    baseChassis.add(turntableBearing);
    for (let i = 0; i < 4; i++) {
        const angle = (i * Math.PI) / 2 + Math.PI / 4;
        const panel = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.28, 0.18), primaryPaint);
    ...
    legConfigs.forEach((cfg, idx) => {
        ...
        hipSocketMount.position.set(Math.cos(cfg.baseAngle) * mountRadius, -0.06, Math.sin(cfg.baseAngle) * mountRadius);
        hipSocketMount.rotation.y = -cfg.baseAngle + Math.PI / 2;
        baseChassis.add(hipSocketMount);
        const bracketMesh = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.26, 0.26), gunmetal);
        ...
        const thighBar = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.18, thighLength), gunmetal);
        thighBar.position.set(0, 0, thighLength * 0.5);
        thighArmature.add(thighBar);
        const thighFlangeTop = new THREE.Mesh(new THREE.BoxGeometry(0.17, 0.04, thighLength * 0.95), darkSteel);
        thighFlangeTop.position.set(0, 0.1, thighLength * 0.5);
        thighArmature.add(thighFlangeTop);
        const thighSideRibL = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.12, thighLength * 0.7), primaryPaint);
        ...
        const pistonCylinder = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, thighLength * 0.65, 12), gunmetal);
        pistonCylinder.rotation.x = Math.PI / 2;
        pistonCylinder.position.set(0, -0.09, thighLength * 0.4);
        thighArmature.add(pistonCylinder);
        const pistonRod = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, thighLength * 0.5, 12), polishedSteel);
        pistonRod.rotation.x = Math.PI / 2;
        pistonRod.position.set(0, -0.09, thighLength * 0.68);
        thighArmature.add(pistonRod);
        const kneeJointDisc = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.13, 0.22, 20), darkSteel);
        ...
        const shinMainBar = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.22, shinLength), gunmetal);
        shinMainBar.position.set(0, 0, shinLength * 0.5);
        shinArmature.add(shinMainBar);
        const shinPlate = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.06, shinLength * 0.8), darkSteel);
        shinPlate.position.set(0, 0.12, shinLength * 0.48);
        shinArmature.add(shinPlate);
        const shinGroove = new THREE.Mesh(new THREE.BoxGeometry(0.165, 0.08, shinLength * 0.5), polishedSteel);
        shinGroove.position.set(0, 0.02, shinLength * 0.45);
        shinArmature.add(shinGroove);
        const ankleJointDisc = new THREE.Mesh(new THREE.CylinderGeometry(0.11, 0.11, 0.18, 16), darkSteel);
        ...
        const heelSpur = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.2, 8), gunmetal);
    ...
    const swivelNeck = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.42, 0.35, 24), darkSteel);
    ...
    const torsoMainSphere = new THREE.Mesh(new THREE.SphereGeometry(0.56, 32, 24), primaryPaint);
    torsoBall.add(torsoMainSphere);
    const equatorBelt = new THREE.Mesh(new THREE.TorusGeometry(0.565, 0.035, 12, 36), darkSteel);
    equatorBelt.rotation.x = Math.PI / 2;
    torsoBall.add(equatorBelt);
    const visorMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.38, 0.36, 0.22, 16, 1, false, -Math.PI * 0.35, Math.PI * 0.7), visorLens);
    visorMesh.position.set(0, 0.05, 0.32);
    visorMesh.rotation.y = 0;
    torsoBall.add(visorMesh);
    const visorEye = new THREE.Mesh(new THREE.SphereGeometry(0.065, 16, 16), opticGlow);
    visorEye.name = 'Node_VisorEye';
    visorEye.position.set(0.12, 0.05, 0.52);
    torsoBall.add(visorEye);
    const auxSensor = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.06, 12), darkSteel);
    ...
    const gunRiser = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.35, 0.22, 20), darkSteel);
    ...
    const receiverBody = new THREE.Mesh(createBeveledCylinder(0.38, 0.42, 0.72, 24), primaryPaint);
    receiverBody.rotation.x = Math.PI / 2;
    gunReceiver.add(receiverBody);
    const receiverEndcap = new THREE.Mesh(new THREE.CylinderGeometry(0.36, 0.36, 0.12, 24), darkSteel);
    receiverEndcap.rotation.x = Math.PI / 2;
    receiverEndcap.position.z = -0.42;
    gunReceiver.add(receiverEndcap);
    const receiverHubCapL = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.22, 0.1, 20), gunmetal);
    ...
    const carryHandle = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.14, 0.45), darkSteel);
    carryHandle.name = 'Node_CarryHandle';
    carryHandle.position.set(0, 0.48, -0.05);
    gunReceiver.add(carryHandle);
    const receiverCollar = new THREE.Mesh(new THREE.CylinderGeometry(0.44, 0.44, 0.14, 24), darkSteel);
    ...
    const barrelCount = 8;
    const barrels = [];
    const centerShaft = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.09, barrelLength + 0.1, 16), polishedSteel);
    centerShaft.rotation.x = Math.PI / 2;
    centerShaft.position.z = barrelLength * 0.5;
    gunRotor.add(centerShaft);
    const rearClampDisc = new THREE.Mesh(new THREE.CylinderGeometry(0.32, 0.32, 0.09, 24), gunmetal);
    rearClampDisc.rotation.x = Math.PI / 2;
    rearClampDisc.position.z = 0.08;
    gunRotor.add(rearClampDisc);
    const midClampDisc = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 0.08, 24), gunmetal);
    midClampDisc.rotation.x = Math.PI / 2;
    midClampDisc.position.z = barrelLength * 0.55;
    gunRotor.add(midClampDisc);
    const frontMuzzleRing = new THREE.Mesh(new THREE.CylinderGeometry(0.31, 0.31, 0.1, 24), darkSteel);
    ...
    for (let b = 0; b < barrelCount; b++) {
        ...
        const barrelTube = new THREE.Mesh(new THREE.CylinderGeometry(barrelRadius, barrelRadius, barrelLength, 14), darkSteel);
        barrelTube.rotation.x = Math.PI / 2;
        barrelTube.position.z = barrelLength * 0.5;
        barrelGroup.add(barrelTube);
        const muzzleTip = new THREE.Mesh(new THREE.CylinderGeometry(barrelRadius * 0.95, barrelRadius * 1.08, 0.08, 14), gunmetal);
        muzzleTip.rotation.x = Math.PI / 2;
        muzzleTip.position.z = barrelLength + 0.04;
        barrelGroup.add(muzzleTip);
        const innerBore = new THREE.Mesh(new THREE.CylinderGeometry(barrelRadius * 0.65, barrelRadius * 0.65, 0.06, 12), new THREE.MeshBasicMaterial({ color: '#000000' }));
    ...
    const runtimeState = {
        ...
        aimTarget: null,
        wearLevel: wear,
        colorScheme: (options.colorScheme || 'hazard-orange'),
        walkTime: 0,
    };
    const passes = {
    ...
    const runtime = {
        ...
        passesReviewed,
        detailInventory,
        playAnimation(name, crossFadeDuration = 0.25) {
            if (runtimeState.currentAnimation === name && name !== 'shoot' && name !== 'stomp')
                return;
        ...
        setJointAngles(angles) {
            ...
            if (angles.legs) {
                angles.legs.forEach((lAngles, i) => {
                    if (legs[i]) {
                        if (lAngles.hipYaw !== undefined)
                            legs[i].hipPivot.rotation.y = lAngles.hipYaw;
                        if (lAngles.hipPitch !== undefined)
                            legs[i].thighPivot.rotation.x = lAngles.hipPitch;
                        if (lAngles.kneePitch !== undefined)
                            legs[i].kneePivot.rotation.x = lAngles.kneePitch;
                        if (lAngles.anklePitch !== undefined)
                            legs[i].anklePivot.rotation.x = lAngles.anklePitch;
                    }
                });
            }
        },
        aimAt(worldTarget, damping = 0.1) {
        ...
        getMuzzleWorldPosition(targetVec = new THREE.Vector3()) {
    ...
    root.userData.tick = (dt) => runtime.tick(dt ?? 0.016);
...
export function tick(group, delta = 0.016) {
...
export function getLookDevLights() {
...
export function createSpiderSentryMech(options = {}) {
    const group = createSpiderSentryMechModel(options);
    return { group, runtime: group.userData.sculptRuntime };
}
