...
export function buildMaterials(wireframe = false) {
    const mats = {
...
export class ParticleEmitter {
    ...
    emit(origin, velocity, scale = 0.08, lifetime = 1.0, color) {
    ...
    burst(origin, count = 12, speed = 1.5, scale = 0.08) {
        for (let i = 0; i < count; i++) {
            const vel = new THREE.Vector3((Math.random() - 0.5) * speed, Math.random() * speed * 0.8 + 0.5, (Math.random() - 0.5) * speed);
...
export class VfxSystem {
    ...
    update(dt, elapsed, sockets) {
        ...
        if (sockets && sockets['Socket_Weapon_Primary']) {
            ...
            if (Math.random() < 0.25) {
                this.sparkleEmitter.emit(swordPos.clone().add(new THREE.Vector3((Math.random() - 0.5) * 0.1, Math.random() * 0.3, (Math.random() - 0.5) * 0.1)), new THREE.Vector3((Math.random() - 0.5) * 0.2, 0.3 + Math.random() * 0.3, (Math.random() - 0.5) * 0.2), 0.05, 0.8);
            }
        }
        // Crown glint pulsing
        if (this.crownGlint && sockets && sockets['Socket_HeadTop']) {
            ...
            this.crownGlint.material.opacity = 0.4 + pulse * 0.6;
        }
    }
    triggerFeatherBurst(pos) {
        this.featherEmitter.burst(pos, 16, 1.8, 0.09);
    }
    triggerDustStamp(pos) {
        this.dustEmitter.burst(pos, 8, 1.2, 0.08);
    }
    triggerVictoryFireworks(origin) {
        for (let i = 0; i < 28; i++) {
            const vel = new THREE.Vector3((Math.random() - 0.5) * 3, Math.random() * 2.5 + 1.5, (Math.random() - 0.5) * 3);
...
export function createSamplingClip(name, duration = 5.0, tracksBuilder) {
...
export function build21Animations() {
 ...
 */
export function createChickenJockeyKingModel(options = {}) {
    ...
    const meshes = {};
    const sockets = {};
    function regMesh(name, mesh, parent) {
        mesh.name = name;
        if (shadows) {
    ...
    function regNode(name, obj, parent) {
        obj.name = name;
        parent.add(obj);
        nodes[name] = obj;
        return obj;
    ...
    const bodyMesh = regMesh('chicken/body/torso', new THREE.Mesh(new THREE.BoxGeometry(0.56, 0.46, 0.68), M.chickenWhite), chickenBody);
    bodyMesh.position.set(0, 0, 0);
    // Under-belly subtle shadow voxel plate
    const bellyMesh = regMesh('chicken/body/belly', new THREE.Mesh(new THREE.BoxGeometry(0.54, 0.08, 0.66), M.chickenWhiteShadow), chickenBody);
    bellyMesh.position.set(0, -0.22, 0);
    // Tail feathers
    const tailMesh = regMesh('chicken/body/tail', new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.18, 0.14), M.chickenWhite), chickenBody);
    ...
    const wingLMesh1 = regMesh('chicken/wings/wing_l_main', new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.32, 0.48), M.chickenWhite), wingL);
    wingLMesh1.position.set(0.04, -0.06, 0.04);
    const wingLMesh2 = regMesh('chicken/wings/wing_l_tier2', new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.24, 0.40), M.chickenWhiteShadow), wingL);
    ...
    const wingRMesh1 = regMesh('chicken/wings/wing_r_main', new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.32, 0.48), M.chickenWhite), wingR);
    wingRMesh1.position.set(-0.04, -0.06, 0.04);
    const wingRMesh2 = regMesh('chicken/wings/wing_r_tier2', new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.24, 0.40), M.chickenWhiteShadow), wingR);
    ...
    const thighLMesh = regMesh('chicken/legs/leg_l_upper', new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.24, 0.05), M.chickenLegYellow), thighL);
    ...
    const shinLMesh = regMesh('chicken/legs/leg_l_lower', new THREE.Mesh(new THREE.BoxGeometry(0.045, 0.22, 0.045), M.chickenLegYellow), shinL);
    ...
    // Three toes
    const footLMid = regMesh('chicken/legs/toe_l_mid', new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.03, 0.18), M.chickenLegYellow), footL);
    footLMid.position.set(0, 0.015, 0.05);
    const footLOut = regMesh('chicken/legs/toe_l_out', new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.03, 0.15), M.chickenLegYellow), footL);
    footLOut.position.set(0.05, 0.015, 0.04);
    footLOut.rotation.y = 0.35;
    const footLIn = regMesh('chicken/legs/toe_l_in', new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.03, 0.15), M.chickenLegYellow), footL);
    ...
    const thighRMesh = regMesh('chicken/legs/leg_r_upper', new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.24, 0.05), M.chickenLegYellow), thighR);
    ...
    const shinRMesh = regMesh('chicken/legs/leg_r_lower', new THREE.Mesh(new THREE.BoxGeometry(0.045, 0.22, 0.045), M.chickenLegYellow), shinR);
    ...
    const footRMid = regMesh('chicken/legs/toe_r_mid', new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.03, 0.18), M.chickenLegYellow), footR);
    footRMid.position.set(0, 0.015, 0.05);
    const footROut = regMesh('chicken/legs/toe_r_out', new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.03, 0.15), M.chickenLegYellow), footR);
    footROut.position.set(-0.05, 0.015, 0.04);
    footROut.rotation.y = -0.35;
    const footRIn = regMesh('chicken/legs/toe_r_in', new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.03, 0.15), M.chickenLegYellow), footR);
    ...
    const headMesh = regMesh('chicken/head/box', new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.38, 0.32), M.chickenWhite), chickenHead);
    headMesh.position.set(0, 0.12, 0);
    // Eyes (Black pixel squares on left and right)
    const eyeL = regMesh('chicken/head/eye_l', new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.09, 0.09), M.zombieEye), chickenHead);
    eyeL.position.set(0.171, 0.16, 0.05);
    const eyeR = regMesh('chicken/head/eye_r', new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.09, 0.09), M.zombieEye), chickenHead);
    ...
    const beakMesh = regMesh('chicken/beak/box', new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.12, 0.18), M.chickenBeakYellow), beak);
    ...
    const wattleMesh = regMesh('chicken/beak/wattle', new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.18, 0.12), M.chickenWattleRed), wattle);
    ...
    const saddleMesh = regMesh('chicken/saddle/leather', new THREE.Mesh(new THREE.BoxGeometry(0.44, 0.06, 0.44), M.saddleLeather), saddle);
    ...
    const zLegLMesh = regMesh('zombie/legs/leg_l', new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.26, 0.14), M.pantsNavy), zLegL);
    zLegLMesh.position.set(0, -0.10, 0);
    const zBootLMesh = regMesh('zombie/legs/boot_l', new THREE.Mesh(new THREE.BoxGeometry(0.145, 0.10, 0.16), M.shoesCharcoal), zLegL);
    ...
    const zLegRMesh = regMesh('zombie/legs/leg_r', new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.26, 0.14), M.pantsNavy), zLegR);
    zLegRMesh.position.set(0, -0.10, 0);
    const zBootRMesh = regMesh('zombie/legs/boot_r', new THREE.Mesh(new THREE.BoxGeometry(0.145, 0.10, 0.16), M.shoesCharcoal), zLegR);
    ...
    const torsoMesh = regMesh('zombie/torso/shirt', new THREE.Mesh(new THREE.BoxGeometry(0.40, 0.38, 0.24), M.shirtCyan), zombieTorso);
    torsoMesh.position.set(0, 0.18, 0);
    // Red Tie
    const tieMesh = regMesh('zombie/tie/body', new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.26, 0.04), M.tieRed), zombieTorso);
    tieMesh.position.set(0, 0.16, 0.13);
    const tieKnot = regMesh('zombie/tie/knot', new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.08, 0.05), M.tieRed), zombieTorso);
    ...
    const cape01Mesh = regMesh('zombie/cape/tier1', new THREE.Mesh(new THREE.BoxGeometry(0.38, 0.24, 0.04), M.capeRed), cape01);
    ...
    const cape02Mesh = regMesh('zombie/cape/tier2', new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.26, 0.035), M.capeRed), cape02);
    ...
    const cape03Mesh = regMesh('zombie/cape/tier3', new THREE.Mesh(new THREE.BoxGeometry(0.46, 0.26, 0.03), M.capeRed), cape03);
    ...
    const sleeveL = regMesh('zombie/arm_l/sleeve', new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.14, 0.14), M.shirtCyan), armL);
    sleeveL.position.set(0, -0.05, 0);
    const armLMesh = regMesh('zombie/arm_l/hand', new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.28, 0.12), M.zombieSkin), armL);
    ...
    const sleeveR = regMesh('zombie/arm_r/sleeve', new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.14, 0.14), M.shirtCyan), armR);
    sleeveR.position.set(0, -0.05, 0);
    const armRMesh = regMesh('zombie/arm_r/hand', new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.28, 0.12), M.zombieSkin), armR);
    ...
    // Handle (Wood)
    const swordGrip = regMesh('zombie/sword/grip', new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.22, 0.06), M.swordHandleWood), swordGroup);
    swordGrip.position.set(0, -0.04, 0);
    // Pommel
    const swordPommel = regMesh('zombie/sword/pommel', new THREE.Mesh(new THREE.BoxGeometry(0.09, 0.08, 0.09), M.swordTealDark), swordGroup);
    swordPommel.position.set(0, -0.16, 0);
    // Stepped Crossguard (Pixelated)
    const swordGuardMid = regMesh('zombie/sword/guard_mid', new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.07, 0.08), M.swordTealDark), swordGroup);
    swordGuardMid.position.set(0, 0.08, 0);
    const swordGuardL = regMesh('zombie/sword/guard_l', new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.08, 0.07), M.swordTeal), swordGroup);
    swordGuardL.position.set(0.14, 0.11, 0);
    const swordGuardR = regMesh('zombie/sword/guard_r', new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.08, 0.07), M.swordTeal), swordGroup);
    swordGuardR.position.set(-0.14, 0.11, 0);
    // Blade Body (Stepped Diamond Voxel Edge)
    const swordBlade = regMesh('zombie/sword/blade_main', new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.68, 0.045), M.swordTeal), swordGroup);
    swordBlade.position.set(0, 0.44, 0);
    // Darker cyan bevel edge on sword
    const swordEdge = regMesh('zombie/sword/blade_edge', new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.64, 0.04), M.swordTealDark), swordGroup);
    swordEdge.position.set(0, 0.44, 0);
    // Pointed Diamond Tip
    const swordTip = regMesh('zombie/sword/blade_tip', new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.12, 0.045), M.swordTeal), swordGroup);
    ...
    const zHeadMesh = regMesh('zombie/face/head_box', new THREE.Mesh(new THREE.BoxGeometry(0.44, 0.44, 0.44), M.zombieSkin), zombieHead);
    zHeadMesh.position.set(0, 0.22, 0);
    // Face Features (Eyes & Screaming Open Mouth)
    const zEyeL = regMesh('zombie/face/eye_l', new THREE.Mesh(new THREE.BoxGeometry(0.09, 0.09, 0.02), M.zombieEye), zombieHead);
    zEyeL.position.set(0.11, 0.24, 0.222);
    const zEyeR = regMesh('zombie/face/eye_r', new THREE.Mesh(new THREE.BoxGeometry(0.09, 0.09, 0.02), M.zombieEye), zombieHead);
    zEyeR.position.set(-0.11, 0.24, 0.222);
    // Screaming Open Mouth Cavity
    const zMouth = regMesh('zombie/face/mouth_open', new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.14, 0.03), M.zombieMouth), zombieHead);
    ...
    // Crown Band
    const crownBase = regMesh('zombie/crown/band', new THREE.Mesh(new THREE.BoxGeometry(0.48, 0.12, 0.48), M.crownGold), crownGroup);
    crownBase.position.set(0, 0.06, 0);
    // 5 Golden Crenellations
    const crenFront = regMesh('zombie/crown/cren_front', new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.16, 0.06), M.crownGold), crownGroup);
    crenFront.position.set(0, 0.18, 0.21);
    const crenFL = regMesh('zombie/crown/cren_fl', new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.14, 0.06), M.crownGold), crownGroup);
    crenFL.position.set(0.18, 0.16, 0.21);
    const crenFR = regMesh('zombie/crown/cren_fr', new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.14, 0.06), M.crownGold), crownGroup);
    crenFR.position.set(-0.18, 0.16, 0.21);
    const crenBL = regMesh('zombie/crown/cren_bl', new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.14, 0.06), M.crownGold), crownGroup);
    crenBL.position.set(0.18, 0.16, -0.21);
    const crenBR = regMesh('zombie/crown/cren_br', new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.14, 0.06), M.crownGold), crownGroup);
    crenBR.position.set(-0.18, 0.16, -0.21);
    // Crown Ruby Inset Gem
    const rubyGem = regMesh('zombie/crown/ruby_front', new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.06, 0.03), M.tieRed), crownGroup);
    ...
    root.userData.sculptRuntime = {
        ...
        animation: {
            playAnimation: (name) => root.userData.sculptRuntime.actions.playAnimation(name),
        },
        vfx: {
            system: vfx,
        },
...
export function createChickenJockeyKingModelLookDevLights(mode = 'reference') {
