...
function createTM01DecalTexture() {
...
function createTreadPatternTexture() {
    if (typeof document === 'undefined')
        return null;
    const canvas = document.createElement('canvas');
    canvas.width = 256;
...
function createHeadlightLensTexture() {
    if (typeof document === 'undefined')
        return null;
    const canvas = document.createElement('canvas');
    canvas.width = 128;
...
function createMicroRoughnessMap() {
    if (typeof document === 'undefined')
        return null;
    const canvas = document.createElement('canvas');
    canvas.width = 128;
...
// -----------------------------------------------------------------------------
function markPickable(obj, partName) {
    obj.name = partName;
    obj.userData.isPickable = true;
    obj.userData.partName = partName;
}
function applyShadowFlags(obj, enabled) {
    obj.traverse((child) => {
        if (child.isMesh) {
...
// -----------------------------------------------------------------------------
export function createChibiTankModel(options = {}) {
    ...
    if (options.turretAngle !== undefined)
        turretPivot.rotation.y = options.turretAngle;
    if (options.cannonElevation !== undefined)
        mantletPivot.rotation.x = options.cannonElevation;
    ...
    const detailInventory = [
        { id: 'turret.tm01_decal', region: 'turret', kind: 'decal', description: 'TM-01 military white stencil', priority: 'high', reviewThreshold: 0.95 },
        { id: 'chassis.treads', region: 'chassis', kind: 'feature', description: 'Dual charcoal track loops with 8 large road wheels', priority: 'high', reviewThreshold: 0.92 },
        { id: 'cannon.barrel', region: 'cannon', kind: 'feature', description: 'Chunky cartoon cannon with flared muzzle and dark bore', priority: 'high', reviewThreshold: 0.94 },
        { id: 'commander.figure', region: 'turret', kind: 'feature', description: 'Commander with helmet, red dot insignia, and goggles', priority: 'high', reviewThreshold: 0.92 },
        { id: 'headlights.dual', region: 'chassis', kind: 'feature', description: 'Dual front headlight pods with glowing amber lenses', priority: 'high', reviewThreshold: 0.9 },
        { id: 'handrails.chrome', region: 'turret', kind: 'feature', description: 'Tubular chrome grab rails on turret deck', priority: 'medium', reviewThreshold: 0.88 },
        { id: 'skirts.latches', region: 'chassis', kind: 'feature', description: 'Side armor skirt latches and hinges', priority: 'medium', reviewThreshold: 0.85 },
        { id: 'chassis.teeth', region: 'chassis', kind: 'feature', description: 'Front lower stepped serrated guard teeth', priority: 'medium', reviewThreshold: 0.85 },
        { id: 'turret.rivets', region: 'turret', kind: 'feature', description: 'Dual rows of side rivets and front cheek rivets', priority: 'medium', reviewThreshold: 0.88 },
        { id: 'hatch.lid', region: 'turret', kind: 'feature', description: 'Hinged open top hatch door with chrome handle', priority: 'medium', reviewThreshold: 0.9 },
    ];
    const landmarks = {
        'cannon.tip': [0.5, 0.38],
...
// -----------------------------------------------------------------------------
export function createChibiTankModelLookDevLights(mode = 'reference') {
